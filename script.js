...
// if we reach a zero, return the string up to that point
function trimTrailingZeros(str) {
  for (let i = str.length - 1; i >= 0; i--) {
    if (str.charAt(i) !== "0") return str.slice(0, i + 1);
  }
  return "";
}
...
function convertToDecimal64() {
  ...
  //int to packed bcd, pads 0s if less than 4 bits
  ...
  document.getElementById("convertButton").addEventListener("click", function () {
    let checkboxState = document.getElementById("exptxt").checked;
    console.log(checkboxState + " wow this");
    if (checkboxState) {
...
function checkInput() {
  ...
  if (!(parseInt(exp).toString() === exp) && !(exp.toLowerCase() === "nan")) {
